...
// 	if( checkVertical(board,column, row, board[column][row]) ) return true;
// 	if( checkHorizontal(board,column, row, board[column][row] ) ) return true;
...
// function checkTurn(board,x,y,lineWin) {
// 	var turn = board[x][y],
// 		won = false,
// 		cV = 1;//, cH = 1, cDl = 1, cDr = 1;

// 		//while ( won == false ) {

// 			// var i=x,j=y, top = 0, bottom=0, cup=0;
// 			// var cVt = true, cVb = true;
// 			// while ( cV <= lineWin && ( cVt || cVb ) ) { // check vertical
				
// 			// 	if ( board[i,j-1] == turn && cVt == true ) {
// 			// 		cV++;
// 			// 	} else {
// 			// 		cVt = false;
// 			// 	}

// 			// 	if (board[i, j+1] == turn && cVb == true ) {
// 			// 		cV++
// 			// 	} else {
// 			// 		cVb = false;
// 			// 	}
...
// 			// if ( cV >= lineWin ) { win = true; continue;} 

// 			for (var i = 1; i  < lineWin; i++) {
// 				for(var j = 1; j < lineWin; j++) { // we stay in range
// 					for( ii =  -1; ii < 1; ii++){
// 						for(jj = -1; jj < 1; jj++){
// 							if ( board[i+ii][j+jj] == turn ) {
// 								console.log("i: "+i+" ii: "+ii+" j: "+j+" jj: "+jj);
// 								console.log("board["+(parseInt(i)+parseInt(ii))+"]["+(parseInt(j)+parseInt(jj))+"] val : +"+board[i+ii][j+jj])
// 							}
// 						}
// 					}
// 				}
...
// }
	...
	function checkHorizontal(board, column, row,turn) {
		...
		for(var j = column + 1; j < 3; j++) {
			console.log("j: "+j+" row : "+row+" board[j][row] ");//+board[j][row]);
