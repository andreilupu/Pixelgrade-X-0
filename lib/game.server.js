...
game_server.getPlayers = function(userid){
	...
	underscore.map(players, function(num){ 
		console.log(num);
		return num;
	});

	//console.log( underscore.toArray(players) );

	//this.players[userid].socket.emit('getPlayers', { players: underscore.flatten(players) });
}
