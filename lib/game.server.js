...
game_server.proccessTurn = function(gameid, host, cellID) {
	...
	//						the board 		coords
	if ( checkFour(this.games[gameid].board, x, y ) ) {
		var winner = "";
		console.log('win');
		if ( host ){
			winner = "host";
...
function checkVertical(board,column, row, turn) {
	...
	var win = false;
	underscore.each(board, function(column, j){
		var vertical = underscore.filter(column, function(num){return num == turn; });
		if ( vertical.length >= 3 ) {	win = true; }
	});
	if ( win ) {
		return true;
	} else {
...
function checkHorizontal(board,column, row, turn) {
	...
	underscore.map(board, function(column, x){
		//console.log(column);
		
		horizontal = [];

		underscore.map(column, function(item, y){
			if ( item == turn ) {
				//console.log("x: "+x);console.log("y: "+y);
				horizontal.join(board[y][y]));
			}
		});
	});

	// horizontal[j] = column[j] ;
